...
import com.tongweb.samples.filewatch.event.Modified;
...
final class WatchingThread extends Thread {
    ...
    private void dispatchEvents(List<WatchEvent<?>> events, MessageEndpointFactory messageEndpointFactory) {
        for (WatchEvent<?> event : events) {
            ...
            out.println("----[RA] Watch thread received event of kind: " + event.kind() + " for " + path.getFileName());
    ...
    private void invoke(final MessageEndpoint endpoint, final Method beanClassMethod, final Path path) throws WorkException {
        ...
        out.println("----[RA] Watch thread scheduling endpoint call via workmanager for method: " + beanClassMethod.getName() + " and file " + path.getFileName());
